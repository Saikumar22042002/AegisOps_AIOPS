...
flowchart TB
    ...
    subgraph CONTROL["FastAPI control plane — 9 routers / 41 endpoints"]
        ...
        APR["POST /approvals/{run_id}<br/>resolve_approval_core: org + state re-check<br/>(single-user HITL)"]
...
- **This install's `.env` diverges from code defaults on three governance-relevant flags**:
  ...
  visible nowhere in the product. [F-9] *(Historical note: the four-eyes concept was REMOVED
  ENTIRELY at the P1 entry gate (2026-08-10) — single-user HITL is THE approval model per 00 §7,
  and neither the flag nor the gate exists in code anymore; the defect that survives is the
  invisibility of flag drift, fixed by governance stamping.)*
...
| D9 | `.env` governance drift (audit-time: four-eyes off, exec loop on, Telegram on). *Superseded in part: the four-eyes concept has since been REMOVED ENTIRELY (P1 entry gate, 2026-08-10) — the drift finding now covers only the exec-loop and Telegram flags, and the underlying fix (stamped, visible governance flags) is unchanged.* | `.env:126,139,146` vs `settings.py:47,49,165` |
...
12. **Governance posture can silently drift via `.env`** — approvers cannot see the active
    governance flags (historically observed with the since-removed four-eyes flag; silent drift
    of any governance flag remains the defect, fixed by governance stamping).
...
strict tenancy/RBAC · single-user human-in-the-loop approval (initiator == approver) · per-step idempotency · boundary-only cancel · honest
