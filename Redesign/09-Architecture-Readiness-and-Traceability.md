...
| R10 | Approvals (durable, four-eyes, deviation) | kept constitution + artifact + args-hash binding | 04 §8.4 | engine approval service | S, T | exists; artifact P3 |
...
| **Four-eyes disabled in shipped `.env`** | D9/F-9 | **BLOCKER (P0)** | Re-enable or record a signed waiver (08 sign-off #5); governance-flag stamping (P0.5) makes any future drift visible on every approval card. |
...
|---|---|---|---|
| **P0 — Protect & repair** | — | eval gate; ledger; defect sweep (D1–D9, F-10); governance stamping + **four-eyes decision**; security hygiene incl. **SA-key + tfstate rotation (BLOCKERs)**; bus default; worker role | CI blocks a deliberately-broken router prompt; spend queryable; BLOCKERs closed with rotation evidence; scenarios G/H pass on current topology |
