...
| R10 | Approvals (durable, single-user HITL — initiator == approver; deviation) | kept constitution + artifact + args-hash binding | 04 §8.4 | engine approval service | S, T | exists; artifact P3 |
...
| **Four-eyes disabled in shipped `.env`** (historical finding) | D9/F-9 | ~~BLOCKER~~ **RESOLVED — concept removed** | Operator directive (P1 entry gate, 2026-08-10): the four-eyes/second-approver concept is removed entirely — no flag, no gate, no UI, no tests. Single-user HITL (initiator == approver) is THE approval model. The surviving requirement is visibility: governance-flag stamping (P0.5) shows the active posture (`approval_model: "hitl"`) on every approval card and `/healthz`. |
