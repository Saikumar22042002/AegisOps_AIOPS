...
|----|------|--------------|-----------------|----|--------|
| S0 | Real multi-tenancy: principal→(org_id,user_id); kill `get_default_org()` everywhere; `Session.user_id`; org predicates; seed **two orgs with users** | `security/deps.py`, `schemas/auth.py`, `db/repositories.py`, all `api/*`, `seed/seed.py` | `test_tenancy.py`: two-org isolation; cross-org UUID → 404; visible in UI | ① | pending |
| S1 | Credential reveal: initiator-or-approver + org-scope + step-up re-auth (≤120s proof) + always-on audit; frontend re-auth modal | `api/artifacts.py`, `security/deps.py`, `frontend/…CredentialReveal` | non-owner → 404; no fresh proof → 401; value once, 2nd → 410; every attempt audited | ②③ | pending |
| S2 | `authorize_run`/`authorize_session` on every read/stream; 404 on mismatch | `api/artifacts.py:_load`, `api/chat.py`, `api/sessions.py` | cross-org read of every tab + stream → 404 | ① | pending |
| S3 | `require_initiator` on `POST /chat` | `api/chat.py` | read-only POST /chat → 403; composer honest for read-only | ② | pending |
| S4 | Redaction backstop on persisted `answer`/`outcome` | `api/chat.py:_persist_result` | planted secret masked in persisted content | — | pending |
| S5 | Capability assertion at the execute node | `agents/execute.py` | execute refuses initiator without capability | — | pending |
...
| HON | Honesty labels: policy "not evaluated"; SRE "proposed, not executed"; Traces "coming — open in Langfuse" deep-link | `agents/templates.py`, `agents/sre.py`, `api/artifacts.py`, frontend | no surface claims what it didn't verify | ⑥⑦⑧ | pending |
| O2 | Langfuse project-key startup assertion | `main.py`/`integrations/langfuse_client.py` | startup asserts key/project | — | pending |
| C1 | Per-cloud plan-assertion tests: ingress/CIDR | tests | SG/NSG/firewall rules asserted from plan JSON per cloud | — | pending |
| D1 | Hot-path indexes | migration, `db/models.py` | `EXPLAIN` shows index scans | — | pending |
| D4 | Repo/state hygiene: purge tracked `*.tfplan`, gitignore, dev TF state off OneDrive | `.gitignore`, compose | `git status` clean; warm plan faster | — | pending |
